// Credits to Jonas Pelzer (https://getslideframe.com)


document.addEventListener('DOMContentLoaded', () => {
    slideFrame = () => {
        attachSlideFrameToLinks(); // Attach the slideFrame logic to all links after page load.
    };
    
    slideFrame();  // Call the function to apply slideFrame when DOM is ready.
});

function attachSlideFrameToLinks() {
    const SFthreshold = 0;
    const SFmain = document.querySelector('.slideframe') || document.createElement('div');
    
    if (!SFmain.classList.contains('slideframe')) {
        SFmain.classList.add('slideframe');
        SFmain.innerHTML = '<div class="slideframe-generalclose"></div><div class="slideframe-container"><div class="slideframe-container-titlebar"><button class="slideframe-container-titlebar-back slideframe-btn">&larr;</button><strong class="slideframe-container-titlebar-title">Title</strong><a href="#" target="_blank" class="slideframe-container-titlebar-external slideframe-btn">&nearr;</a></div><iframe class="slideframe-container-frame" src=""></iframe></div>';
